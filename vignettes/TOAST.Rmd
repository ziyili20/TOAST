...
thus `findRefinx_cv()` is recommended.
For log-counts, the variance-mean correlation 
largely disappears, so both `findRefinx_cv()` and `findRefinx_var()`
would work similarly. In DNA methylation data, this correlation is not 
strong, either `findRefinx_cv()` or `findRefinx_var()`
can be used. In this case,  
we recommend `findRefinx_var()` because we find it
